...
DELIVERABLES = [
    ...
    dict(name="Fleet Skills, Scripts & Jobs Distribution", type="Agent Platform", status="Live", health="Healthy", functions=["Company"], audience="Company", owner="Wes / LT", purpose="Distributes common and function-scoped skills, shared scripts, and managed jobs from the public pcg-agents repo.", schedule="Every 30 minutes per member box", url="https://github.com/Pro-Coffee-Gear/pcg-agents", job="pcg-fleet-sync (per member box)", scripts="pcg_sync.py", visibility="Team", notes="Public repo reads use no GitHub credential; repo-managed assets use the pcg- prefix."),
